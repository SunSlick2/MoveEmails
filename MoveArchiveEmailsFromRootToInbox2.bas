Sub MoveArchiveRootItemsToInbox()
    ...
    Application.StatusBar = "Starting migration... 0 of " & lInitialCount & " items moved."
    Debug.Print "Starting migration: Moving " & lInitialCount & " items from root to Inbox."
    ...
    For i = lInitialCount To 1 Step -1
        ...
        If objItem.Class = olMail Then
            ...
            If Err.Number <> 0 Then
                ...
                Debug.Print "ERROR moving item '" & objItem.Subject & "'. Error: " & Err.Description
                Err.Clear ' Clear the error state to allow the loop to continue
            Else
                lMovedCount = lMovedCount + 1
                ' Update the status bar frequently
                Application.StatusBar = "Moving items... " & lMovedCount & " of " & lInitialCount & " items moved."
            End If
        Else
    ...
    MsgBox "Migration complete!" & vbCrLf & _
           ...
           "Failed/Skipped: " & (lInitialCount - lMovedCount), vbInformation, "Migration Summary"
